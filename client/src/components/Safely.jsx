...
class Safely extends React.Component {
	...
	addUser() {
		// console.log(this.state.addUserBool);
		this.setState({addUserBool: !this.state.addUserBool});
	}

	saveNumber() {
		// TODO: Make sure that each phone number is unique.

		console.log('save number here');
		//save to database the name and phone of the added user associated with logged in user
		//axios post req to server here '/saveNumber'
	  ...
	  .catch(function (error) {
		...
		//get the value {add} from the inputs and store it to a database
		//show the updated state to the user
	}
	...
	checkBoxClicked(e) {
		...
		if (e.target.checked) {
			...
			//remove (make this not an array later)
		...
		//add all the numbers to selectedNumbers
	}
	...
	send() {
		...
		console.log('here are all the numebrs to send to: ', this.state.selectedNumbers);
	  ...
	  .catch(function (error) {
		...
		//send this.state.selectedNumbers/ this.state.message
	}
	...
	//keep track of all the checked rows
	render() {
...
// ReactDOM.render(<Safely />, document.getElementById('app'));